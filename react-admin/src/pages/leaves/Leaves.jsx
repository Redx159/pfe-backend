...
export default function Leaves() {
  ...
  if (loading) return <><Navbar /><div style={{ padding: 20 }}>{t("common.loading")}</div></>;
  ...
  return (
    <><Navbar /><div style={{ padding: 20 }}>
      ...
      <div style={{ border: "1px solid #d8dee4", borderRadius: 12, padding: 14, marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {upcomingLeaves.map((leave) => (
              <div key={leave.id} style={{ borderTop: "1px solid #eaeef2", paddingTop: 10 }}>
                ...
                <div>{leave.start_date} → {leave.end_date} ({leave.leave_type})</div>
                <div style={{ color: leave.status === "APPROVED" ? "#1a7f37" : leave.status === "PENDING" ? "#9a6700" : "#cf222e", fontWeight: "bold" }}>{leave.status}</div>
              </div>
            ))}
          </div>
        ) : (
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <input placeholder={t("common.searchEmployee")} value={search} onChange={(e) => setSearch(e.target.value)} style={{ marginLeft: 10 }} />
      </div>
        ...
        <tbody>
          {leaves.filter((leave) => {
            ...
            <tr key={leave.id}>
              ...
              <td>{leave.reason}</td>
              <td>{leave.attachment_url ? <a href={leave.attachment_url} target="_blank" rel="noopener noreferrer">{t("leaves.viewAttachment")}</a> : "-"}</td>
              <td><span style={{ fontWeight: "bold", color: leave.status === "APPROVED" ? "green" : leave.status === "REJECTED" ? "red" : leave.status === "PENDING" ? "orange" : "gray" }}>{leave.status}</span></td>
              <td>{leave.manager_comment || "-"}</td>
              <td>
                {leave.status === "PENDING" && (
                  <>
                    <button onClick={() => handleApprove(leave.id)}>{t("common.approve")}</button><br />
                    <textarea value={comments[leave.id] || ""} onChange={(e) => setComments({ ...comments, [leave.id]: e.target.value })} />
                    <button onClick={() => handleReject(leave.id)}>{t("common.reject")}</button>
                  </>
                )}
              </td>
            </tr>
